...
- **动画开关**：可在配置文件中通过 `enable_animations` 开启/关闭。
- **动画时长**：可在配置文件中通过 `animation_duration` 调整时长（建议 0.5s - 1.0s）。
- **配置优先级**：Markdown 文件中的指令 > 配置文件中的 `transition_type`。

#### 转场指令

可以使用 HTML 注释在 Markdown 中动态控制转场效果：
...
**逻辑规则**：
- 支持 `fall`（坠落）和 `glitch`（故障艺术）两种类型。
- 指令具有**持久性**：一旦设置，后续所有页面都将使用该转场效果，直到遇到下一个指令。
- 建议在幻灯片开头或分割线 `---` 后立即设置。
...
- **切出效果**：当前幻灯片的字符以“故障艺术”的形式，伴随随机跳闪、风格反转、颜色突变后逐渐消失。
- **切入效果**：新幻灯片的字符以“故障艺术”的形式，从跳闪状态逐渐稳定到正常位置和样式。
- **特性**：
  - 非整体跳闪：每个字符（或词）根据其独立的随机延迟进入故障状态，实现局部异步跳闪效果。
  - 视觉元素：包含随机字符替换（如 `░`, `▒`, `▓`, `█`）、位置偏移、颜色反转等。
  - 动态感：基于 20FPS 高频刷新，营造极具科技感与不稳定的视觉体验。
